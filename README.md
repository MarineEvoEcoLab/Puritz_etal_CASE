...
> The *C. virginica* exon annotation (`sorted.ref3.0.exon.bed`) that the pipeline intersects against is not tracked in this repository — at 183 MB it exceeds GitHub's file-size limit. Regenerate it from the GCF_002022765.2 GFF, or fetch it from wherever the lab hosts large files (Zenodo/LFS).
...
Effective population size and per-spawn q thresholds are set per spawn to account for differences in breeder number and coverage (e.g., Ne = 250 for SP2 vs. 1,000 for SP3/SP4). See the analysis notebook for exact values.
...
**R analysis.** The pipeline was run under R 3.6.0. Render the main notebook from the repository root:
...
> Raw sequencing reads are not yet deposited; an NCBI SRA/BioProject accession will be added here once available. Large intermediate files are not tracked in this repository.
